...
void SdoReplyCallback(const mcs_interface::CiA_SdoMessage::ConstPtr& msg){
  ...
  if(handle.AccessResult==0){
    ...
    switch (handle.AccessResult) {
      case OBD_ACCESS_ERR_WRITE:
          printf("sdo access failed: write error (%d)\n", handle.AccessResult);
        break;
      case OBD_ACCESS_ERR_READ:
        printf("sdo access failed: read error (%d)\n", handle.AccessResult);
        break;
      case OBD_ACCESS_ERR_IDX_NONEXIST:
        printf("sdo access failed: unknown object index (%d)\n", handle.AccessResult);
        break;
      case OBD_ACCESS_ERR_DATA_RANGE:
        printf("sdo access failed: data range error (%d)\n", handle.AccessResult);
        break;
    }
  }
}
...
int main(int argc, char **argv){
  ...
  ros::init(argc, argv, "mcs_obd_read");
  ros::NodeHandle node;

  std::string idx, subidx;
  if(argc != 3){
     printf("Usage: \n    rosrun mcs_interface mcs_obj_read 0xHHHH 0xHH\n");
     return 0;
  } else {
    idx = argv[1];
    subidx = argv[2];
